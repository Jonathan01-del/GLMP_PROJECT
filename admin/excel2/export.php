...
if(isset($_POST["export"]))
{
  ...
  $connect=mysqli_connect("localhost","root","","vx_report");
 // $tblname=mysqli_real_escape_string($connect,$_POST['']);
  $query="SELECT * FROM tbl_exhibitor";
  if ($result=mysqli_query($connect,$query))
  {
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount,"Exhibitor_name"); 
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount,"SHOW_JOINED");
    $rowCount++;
    //Adding Data to Sheet
    while($row = mysqli_fetch_array($result))
    {
      $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $row['Exhibitor_name']);
      $objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $row['SHOW_JOINED']);
      $rowCount++;
    }
