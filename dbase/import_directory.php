            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
                ...
                if ($qryRowComp->num_rows > 0) {
                    ...
                    if ($qryproResult->num_rows > 0) {
                        ...
                        $db->query("UPDATE tbl_company_directory SET directory_id=$directory WHERE company_id='$get_comp_id' ");
                        

                    }else{

                        // Insert tbl_company_directory
                        $db->query("INSERT INTO tbl_company_directory (company_id, directory_id, encoder_id, date_last_updated, date_created) VALUES ('$get_comp_id','$directory', '$encoder', '$last_update', NOW())");

                    }
                    # End  Query to Insert the tbl_company_directory

                } else {
                    // Insert company data in the database
                    $insert_company = "INSERT INTO tbl_company (company_name, address, city, country, telephone, fax, email , product_profile, main_line_of_business, website, directory_id, date_created) VALUES ('$company', '$address', '$city', '$country', '$tel', '$fax', '$email','$prod_profile', '$line_Bus', '$website', '$directory', NOW())";
                    ...
                    if ($db->query($insert_company) === TRUE) {
                        ...
                        $db->query("INSERT INTO tbl_company_directory (company_id, directory_id, encoder_id, date_last_updated, date_created) VALUES ('$comp_id','$directory', '$encoder', '$last_update', NOW())");
